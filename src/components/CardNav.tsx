...
const CardNav = ({
  ...
  const calculateHeight = () => {
    ...
    if (isMobile) {
      ...
      if (contentEl) {
        ...
        const topBar = 60;
        const padding = 20;
        const contentHeight = Math.max(contentEl.scrollHeight, 180);
  ...
  return (
    <div
      className={`card-nav-container fixed left-1/2 -translate-x-1/2 w-[95%] max-w-[800px] z-[99] top-[1.2em] md:top-[2em] md:w-[90%] ${className}`}
    >
      <nav
        ref={navRef}
      ...
      >
        ...
        <div
          className={`card-nav-content absolute left-0 right-0 top-[60px] bottom-0 p-3 flex flex-col items-stretch gap-2 justify-start z-[3] ${
            isExpanded ? 'visible pointer-events-auto' : 'invisible pointer-events-none'
          } md:flex-row md:items-end md:gap-[12px] md:p-2`}
            ...
            >
              ...
              <div className="nav-card-links mt-auto flex flex-col gap-[2px]">
                {item.links?.map((lnk, i) => (
                  <button
                    key={`${lnk.label}-${i}`}
                    className="nav-card-link inline-flex items-center gap-[6px] no-underline cursor-pointer transition-opacity duration-300 hover:opacity-75 text-[15px] md:text-[16px] bg-transparent border-none text-left p-0"
